...
export interface MediaTray {
  ...
  items: MediaItem[];
  /** Every file this browser has sent to local CUA storage, newest first. */
    ...
    try {
      ...
      onError(
        caught instanceof Error
          ? `Upload failed — ${caught.message}. Check the file and retry.`
        ...
        <li key={item.id} className="ohf-strip-item">
          <span className="ohf-strip-tile">
            {item.role === "audio" || item.role === "video" ? (
              ...
              /* Local user upload; a plain img avoids proxying it again. */
